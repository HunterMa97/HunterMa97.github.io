...
function attentionField(canvasId, opts = {}) {
  const canvas = document.getElementById(canvasId);
  if (!canvas || reduceMotion) return;
  const ctx = canvas.getContext("2d");
  const pointer = { x: -9999, y: -9999, active: false };
  const particles = [];
  const cfg = { spacing: 150, linkR: 130, repelR: 55, attract: 0.002, ...opts };
  ...
  function resize() {
    ...
    particles.length = 0;
    for (let x = 40; x < rect.width; x += cfg.spacing) {
      for (let y = 40; y < rect.height; y += cfg.spacing) {
        particles.push({ x: x + Math.random() * 36, y: y + Math.random() * 36, vx: 0, vy: 0, homeX: x, homeY: y });
      }
    }
  }
  ...
  function tick() {
    ...
    const color = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#b31b1b";
    particles.forEach((p, i) => {
      let ax = (p.homeX - p.x) * 0.0008;
      let ay = (p.homeY - p.y) * 0.0008;
      particles.forEach((q, j) => {
        ...
        if (d < cfg.linkR) {
          const alpha = Math.max(0, 1 - d / cfg.linkR) * 0.13;
          ctx.strokeStyle = color.replace("rgb", "rgba").replace(")", `, ${alpha})`);
          if (!ctx.strokeStyle.includes("rgba")) ctx.strokeStyle = `rgba(179,27,27,${alpha})`;
          ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(q.x, q.y); ctx.stroke();
          ax += dx * cfg.attract * (1 - d / cfg.linkR);
          ay += dy * cfg.attract * (1 - d / cfg.linkR);
        }
        if (d < cfg.repelR) {
          ax -= dx / d2 * 0.35;
          ay -= dy / d2 * 0.35;
  ...
  canvas.addEventListener("click", (event) => {
    const rect = canvas.getBoundingClientRect();
    particles.push({ x: event.clientX - rect.left, y: event.clientY - rect.top, vx: 0, vy: 0, homeX: event.clientX - rect.left, homeY: event.clientY - rect.top });
  });
  window.addEventListener("resize", resize);
  resize();
  tick();
}
attentionField("tokenCanvas");
attentionField("contactCanvas", { spacing: 190, linkR: 150 });
